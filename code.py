...
def main():
    ...
    company_data = pd.read_csv("https://raw.githubusercontent.com/That-Tech-Geek/STOC/main/Directory%20-%20Sheet1%20(1).csv")
    ...
    if company_name:
        try:
            ...
            if option == 'Close':
                ...
                ax.tick_params(axis='y', colors='white')  
                ax.grid(True, color='white', linestyle='--', linewidth=0.5)  
                ax.plot(data_filtered['Date'], data_filtered['Close'], color='blue', label='Close')  
                ax.plot(data_filtered['Date'], data_filtered['Adj Close'], color='red', label='Adj Close')  
                ax.set_title(f'{ticker_symbol} {column_titles[option]} Over Time', color='white')  
                ax.set_xlabel('Date', color='white')  
